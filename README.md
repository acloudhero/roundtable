...
**Version:** `0.12.0` (PWA Implementation Bundle) · Schema: `0.11.0` · Storage key: `roundtable.appState.v1`
...
> **v0.12.0 — PWA Implementation Bundle.**
> RoundTable is now installable and offline-capable as a static PWA
> while preserving the no-backend, no-API, local-first workflow boundary.
> See `docs/PWA.md` for the operator guide and `netlify.toml` for the
> current static hosting configuration.

> **v0.11.0 — Markdown Handoff Mode (Release Candidate).**
> A parallel, file-mediated substrate to the v0.10.x clipboard
> workflow. Every Context-Sandwich prompt, model response, mediator
> packet, and structured synthesis can now be exported as a `.md`
> file with locked YAML frontmatter (provenance + integrity hashes)
> and re-imported with a preview gate, stale-state detection, and
> snapshot-rollback. A universal Raw Notes fallback preserves any
> body the importer can't safely commit. **No new network or
> backend surface** — every file moves via the operator's local
> filesystem and the OS file picker. New top-level `AppState`
> arrays: `rawNotes` (200-entry ring buffer) and `importHistory`
> (50-entry ring buffer with rollback snapshots). Migration step
> defaults both to `[]` for older states.
>
> See `docs/MARKDOWN_HANDOFF.md` for the operator reference and
> `docs/RELEASE_CHECKLIST.md → v0.11.0 Acceptance Walk` for the
...
Current: **v0.12.0 — PWA Implementation Bundle (Checkpoint K)**

Last operational phase: Phase 9 — Release Candidate Hardening and RoundTable Rename.
