# Gemini Review Packet — Phase 7C Claude Opus Prompt
...
**Purpose of this file:** Give Gemini a clean prompt to critique before the user sends it to Claude Opus 4.7.  
**Status:** Prompt-review packet only. Do not treat this as implementation authorization.
...
Your task is to critique the Claude Opus prompt below before it is sent to Opus.

Do not implement anything. Do not rewrite the project. Your job is to determine whether the prompt is safe, properly scoped, architecture-compliant, and complete enough to produce a high-quality Phase 7C Architecture Baseline Amendment candidate.

Pay special attention to:
- Whether the prompt accidentally authorizes implementation.
- Whether the prompt preserves the current Architecture Baseline constraint that app containerization is not yet allowed until amended.
- Whether the prompt keeps Phase 7C as planning/amendment work only.
- Whether the prompt adequately covers SQLite WAL, Docker volumes, artifact persistence, blue/green topology, front-door behavior, OTel Collector topology, resource attributes, and future Phase 8 telemetry fan-out implications.
- Whether the prompt avoids vendor secrets, vendor SDKs, fake telemetry validation, cloud deployment, Kubernetes, Terraform, production auth, and multi-tenant behavior.
- Whether the prompt gives Opus enough structure without letting Opus overbuild.
...
### Executive Verdict
State whether this prompt is safe to send to Claude Opus as-is, safe with edits, or unsafe.

### Prompt Scope Assessment
Evaluate whether the prompt correctly limits Opus to Phase 7C planning/amendment only.

### Architecture Baseline Compliance
Evaluate whether the prompt respects the current baseline and handles amendment mechanics correctly.

### Containerization Design Coverage
Evaluate whether the prompt asks for the right containerization design topics.

### SQLite / WAL / Volume Risk Review
Evaluate whether the prompt forces Opus to address data-loss and file-locking risks.

### Blue/Green Topology Review
Evaluate whether the prompt correctly asks Opus to handle blue/green slot separation, active-slot routing, and rollback.

### OpenTelemetry / Resource Identity Review
Evaluate whether the prompt properly addresses resource attributes, Collector topology, and future Dynatrace/entity mapping.

### Local-First and Security Review
Evaluate whether the prompt preserves local-first defaults and avoids secrets/cloud/vendor risks.

### Hidden Implementation Leakage
Identify any wording that could cause Opus to start writing code, Dockerfiles, compose files, or implementation patches.

### Missing Questions or Requirements
List anything the prompt should add before being sent to Opus.

### Recommended Edits
Provide specific edits, additions, or removals.
...
11. How should `runs/` be mounted or persisted?
12. How should `feed/` be mounted or persisted?
13. Should blue and green slots have separate artifact roots?
14. Should published feed output be shared, copied, promoted, or slot-scoped?
15. What must be true for rollback not to corrupt feed/audio state?
...
Write a user-reviewable proposed canonical state update.
